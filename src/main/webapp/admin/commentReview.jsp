...
<script type="text/javascript">
	...
	function commentReview(state){
		...
		$.messager.confirm("系统提示","您确定要审核这<font color=red>"+selectedRows.length+"</font>条评论吗？",function(r){
			if(r){
				$.post("${pageContext.request.contextPath}/admin/comment/examine.do",{ids:ids,state:state},function(result){
					if(result.success){
	...
	function formatBlogTitle(val,row){
		if(val==null){
			...
			return "<a target='_blank' href='${pageContext.request.contextPath}/blog/articles/"+val.id+".ac'>"+val.title+"</a>";
		}
	}
